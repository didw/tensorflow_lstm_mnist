...
class TensorflowLSTM():
    def __init__(self, h_size=128, n_inputs=28, n_steps=28, n_classes=10, l_r=0.001):
        ...
        self.Y = tf.placeholder(tf.float32, [None, n_steps, n_classes])
        ...
        outputs = tf.reshape(outputs, [-1, h_size])
        self.pred = tf.matmul(outputs, w1) + b1
        self.pred = tf.reshape(self.pred, [-1, n_steps, n_classes])
        self.cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=self.pred, labels=self.Y))
        tf.summary.scalar('loss', self.cost)
        self.train_op = tf.train.AdamOptimizer(l_r).minimize(self.cost)

        correct_pred = tf.equal(tf.argmax(self.pred,2), tf.argmax(self.Y,2))
    ...
    def fit(self, mnist, n_epoch=10, batch_size=128):
        ...
        test_y = np.transpose([test_y] * self.n_steps, axes=(1,0,2))
        acc = 0
        with tf.Session(config=config) as sess:
            ...
            for i in range(n_epoch):
                ...
                for j in range(int(60000/batch_size)):
                    ...
                    batch_y = np.transpose([batch_y] * self.n_steps, axes=(1,0,2))
...
def main():
    ...
    print("accuracy: ", np.mean(np.equal(np.argmax(pred[:,-1,:],1), np.argmax(test_lab,1)))*100)
